...
// import './data-sources' // Temporary notes，File does not exist
...
// 🔥 supplyEditorDataSourceManagerto child component（Temporary notes，File does not exist）
...
// 🔥 Pass the component executor registry toEditorDataSourceManager（Temporary notes）
// editorDataSourceManager.setComponentExecutorRegistry(componentExecutorRegistry.value)

// initialization Card 2.1 component tree system
...
const props = defineProps<{
   ...
   * Default is true，Stay backwards compatible
   */
  showToolbar?: boolean
  /**
...
// drawer status - initial state：preview mode，Drawer closed
...
const showWidgetTitles = ref(true) // main switch，Show title by default

// Multiple data sources data storage - with componentsIDas key
const multiDataSourceStore = ref<Record<string, Record<string, any>>>({})

// Multiple data source configuration storage - with componentsIDas key，Store complete configuration information
...
// 🔥 supplyvisualEditorStateto interactive system components
provide('visualEditorState', {
  /**
   * Get all components in the current editor
   * Returns the formatted component list，For use by interactive systems
   */
  getAvailableComponents: () => {
    // fromeditorStoreGet all current nodes
    const nodes = stateManager.nodes || []
    return nodes.map(node => ({
      id: node.id,
      type: node.type,
      title: node.label || node.metadata?.title || `components (${node.id.slice(0, 8)})`,
...
// 🔥 Poll management combined functions
const pollingManagerDependencies = {
  pollingManager,
  stateManager,
  configurationManager
  // editorDataSourceManager // Temporary notes，File does not exist
}
const {
...
// Using state variables from the data manager（Replace the original）
...
// 🔥 Event handling combined functions
...
// 🔥 Lifecycle management combined functions
// Event listener reference，for cleaning
...
// Monitor changes in selected components，Add anti-shake
watch(
...
const handleModeChange = (mode: 'edit' | 'preview') => {
  if (mode === 'edit') {
    ...
    // 🎯 Improve user experience：Automatically open the left component library drawer when entering edit mode
    ...
    if (JSON.stringify(currentState) !== JSON.stringify(preEditorConfig.value)) {
      dialog.warning({
        ...
        onPositiveClick: () => {
          // User confirms exit，Reset configuration
          ...
          // important：make sureeditorConfiginisEditingforfalse，Avoid state conflicts
          editorConfig.value = { ...preEditorConfig.value, isEditing: false }
          // important：callingsetStateBefore，make surepreEditorConfiginisEditingforfalse
          const resetConfig = { ...preEditorConfig.value, isEditing: false }
          setState(resetConfig)
        },
        onNegativeClick: () => {
          // User cancels exit，keep current status，Do nothing
        }
      })
    } else {
...
// Import and export、Event handling functions such as view control have been migrated to usePanelEventHandler

// Canvas configuration、Multiple data source processing functions have been migrated to usePanelEventHandler

/**
 * 🔥 New：Handle configuration panel request for current data
 * Provide the current runtime data of the component to the configuration panel，Implement the memory data priority principle
 */
const handleRequestCurrentData = (widgetId: string) => {
  ...
  if (currentConfig) {
    // pass ConfigurationManager Temporarily update configuration，Make the configuration panel available
...
/**
 * 🔥 New：Handling data source manager update events
 * Receive data source configuration updates from the configuration panel，and sync to the editor data source manager
 * Temporary notes：EditorDataSourceManager File does not exist
 */
const handleDataSourceManagerUpdate = (updateData: {
  ...
  // Temporarily comment out the entire function implementation
  ...
  try {
    ...
    // 🔥 protection：Make sure the editor data source manager is initialized and the component exists
    ...
    // 🔥 protection：Don’t affect the overall process when making mistakes，Only log errors
  }
  */
}

// Canvas operation control、Renderer event handling、Node selection and interaction functions have been migrated to usePanelEventHandler

// save panel
const handleSave = async () => {
  // Check if it isCanvasRenderer，If so, a developing prompt will be displayed.
  ...
  try {
    ...
    // 🔥 unified format：Save directly in simple format，Both old and new versions can be read
    const { error } = await PutBoard({
      id: props.panelId,
      config: JSON.stringify(currentState), // save directly {widgets: [], config: {}}
...
// Monitor status changes，Automatically set hasChanges
watch(
  [
...
/**
 * Monitor component node changes，Automatically sync data source manager
 * Temporary notes：EditorDataSourceManager File does not exist
 */
/*
watch(
 ...
 * Listening to data source manager events，Implement data distribution
 */
const setupDataSourceEventListeners = () => {
  ...
  try {
    ...
    dataUpdateListener = (eventData: { componentId: string; result: any }) => {
      ...
      // 🔥 Performance optimization：Reduce unnecessary console output

      if (result.success && result.data) {
        try {
          // 🔥 Performance optimization：Check if the data actually changed，Avoid unnecessary responsive updates
          ...
          if (hasDataChanged) {
            // 🔥 key：Distribute data to multiDataSourceStore，for component use
            multiDataSourceStore.value[componentId] = result.data
          } else {
            // Data has not changed，Skip updates
          }

          // Tags have changed（Optional，Depends on whether you want data updates to trigger a save prompt）
          // hasChanges.value = true
        } catch (error) {}
      } else {
      }
    ...
    statusChangeListener = (eventData: { componentId: string; status: string; error?: string }) => {
      ...
      // 🔥 Performance optimization：Only output status change logs in the development environment

      if (error) {
        // 🔥 TODO: User friendly error notifications can be added hereUI
    ...
    pollingStatusListener = (eventData: { componentId: string; isPolling: boolean }) => {
      ...
      // 🔥 Performance optimization：Only output polling status log in development environment
    }
 ...
 * Adding a listening component、delete、Configuration changes and other events
 */
...
const syncDataSourceConfigs = async () => {
  ...
  return
  try {
    // Traverse all nodes，Check if there is data source configuration
...
/**
 * 🔥 Add dual data source test components developed according to the new architecture
 * add a dual-data-display components，Configure two data sources for testing the new architecture
 */
const addNewArchitectureTestComponent = async () => {
  try {
    ...
    const testComponent: VisualEditorWidget = {
      ...
      config: {
        // 🔥 Configure two data sources for testing the new architecture
        dataSources: {
          dataSource1: {
            type: 'static',
            enabled: true,
    ...
    // Add components to the canvas（Only pass type string and position）
    await addWidget('dual-data-display', { x: 50, y: 50 })

    // Get the newly added componentID（fromstateManager.nodesFind the latest in）
...
// Component lifecycle management（Restoring a multiple data source configuration、Event listener cleanup、onUnmounted）Migrated to usePanelLifecycleManager
</script>
...
<style scoped>
/* 🔥 Core fix：PanelEditor Root container height settings */
.panel-editor-root {
  width: 100%;
  height: 100%;
...
.panel-editor {
  min-height: 600px;
  /* 🔥 strengthen：Make sure the editor is tall enough in any case */
  height: 100%;
}

.h-edit-area {
  height: calc(100% - 30px);
  /* 🔥 critical fix：When the height of the parent container is unclear，Use viewport height as baseline */
  min-height: calc(100vh - 160px); /* minus the head、Navigation and other fixed heights */
}
...
.canvas-container {
  ...
  width: 100%;
  height: 100%;
  /* 🔥 critical fix：Canvas container sets minimum height，Make sure the grid area is visible */
  min-height: 600px;
  overflow: hidden;
...
.renderer-container {
  width: 100%;
  height: 100%;
  /* 🔥 Grid layout requires fixed height，Cannot be usedoverflow:auto */
  min-height: 500px;
  position: relative;
}
...
/* Special style of left drawer - Translucent effect，Does not affect dragging */
:deep(.n-drawer--left) {
