...
- [x] will old version `.gs-xx` Replace injection logic with dependencies GridStack own style injection，and eventually merged into a new streamlined version。
- [x] GridV2 completely refactored：initialization、Column number switching、layout Synchronize all transfers GridStack Native API deal with，Remove custom targeting and redundant events。
- [x] wrapper no longer here `gridConfig` Rebuild on change layout，Sync drag only/scaling configuration；spacing handed over `margin` manage。
