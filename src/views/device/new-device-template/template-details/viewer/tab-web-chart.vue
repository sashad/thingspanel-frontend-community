...
/**
 * Web Chart configuration Tab
 * use PanelEditorV2 Make visual edits
 * Data format：{ version: "v2", web: { config: {...} }, app: { config: {...} } }
 * save to web_chart_config field web part
 */
...
// 🔥 Is it an old version of data?（No v2 Format）
...
/**
 * 🔥 parse web_chart_config and extract Web Configuration
 */
const parseWebChartConfig = () => {
  ...
  try {
    ...
    console.error('❌ parse web_chart_config fail:', e)
 ...
 * save to web_chart_config of web part
 */
const handleCustomSave = async (editorState: any) => {
  ...
  saving.value = true
  try {
    // 1. Read existing configuration（reserve app part）
    ...
    if (templateData.value?.web_chart_config) {
      try {
        ...
        // Legacy data or parsing failure，neglect
      }
    }

    // 2. renew web part（Save the complete data generated by the Kanban board）
    existingConfig.web = {
...
const handleSaveError = (error: any) => {
  console.error('save Web Chart configuration failed:', error)
    ...
    <NSpin :show="saving">
      ...
      <div v-if="isLegacyData" class="legacy-data-warning">
        <NAlert type="warning" :title="$t('device_template.legacyDataDetected') || 'Legacy data detected'" closable>
          <template #default>
            {{ $t('device_template.legacyDataMessage') || 'Current configuration is in legacy format，Cannot edit directly。Please recreate the configuration。' }}
          </template>
          <template #action>
